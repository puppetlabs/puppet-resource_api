...
RSpec.describe Puppet::ResourceApi::Transport do
  ...
  context 'when registering a transport' do
    ...
    it 'adds to the transports register' do
      ...
      expect(described_class.transports).not_to be_nil
      expect(described_class.transports['a_remote_thing']).to be_a Puppet::ResourceApi::TransportSchemaDef
      expect(described_class.transports['a_remote_thing'].definition).to eq schema
    end
  end
